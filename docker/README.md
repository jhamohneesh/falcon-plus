...
```
    ...
    cd /tmp && \
    git clone --depth=1 https://github.com/jhamohneesh/falcon-plus && \
    cd /tmp/falcon-plus/ && \
...
```
    ...
    docker run -itd --name falcon-plus \
         ...
         -v /home/work/jhamohneesh/data:/jhamohneesh/data \
         -v /home/work/jhamohneesh/logs:/jhamohneesh/logs \
    ...
    docker exec falcon-plus ./jhamohneesh check
    
    ## or you can check logs at /home/work/jhamohneesh/logs/ in your host
    ls -l /home/work/jhamohneesh/logs/
...
```
    docker run -itd --name falcon-dashboard \
        ...
        -w /jhamohneesh/dashboard openfalcon/falcon-dashboard:v0.2.1  \
       './control startfg'
```
...
```
    sudo docker run -d --restart always --name falcon-agent -e NUX_ROOTFS=/rootfs -v /:/rootfs:ro openfalcon/falcon-plus:v0.3 ./agent/bin/falcon-agent -c /jhamohneesh/agent/config/cfg.json
```

----

## Building jhamohneesh images from source code
...
=======
    cd /tmp && \
    git clone https://github.com/jhamohneesh/dashboard  && \
    cd /tmp/dashboard/ && \
    docker build -t falcon-dashboard:v0.2.1 .
```
