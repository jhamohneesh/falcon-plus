...
.PHONY: $(TARGET)
$(TARGET): $(GOFILES)
	go build -ldflags "-X main.BinaryName=Open-Falcon -X main.GitCommit=`git rev-parse --short HEAD` -X main.Version=$(VERSION)" -o jhamohneesh

checkbin: bin/ config/ jhamohneesh
...
pack: checkbin
	...
	tar -C out -zcf jhamohneesh-v$(VERSION).tar.gz .
	@rm -rf out
...
clean:
	...
	@rm -rf ./out
	@rm -rf ./$(TARGET)
	@rm -rf jhamohneesh-v$(VERSION).tar.gz
